...
# Thème sombre calqué sur l'app MT5 mobile (partagé par tous les graphiques)
_MT5_BG = "#000000"
...
def _dark_candles(ax, df: pd.DataFrame) -> None:
    """Bougies style MT5 (violet=haussière, blanc=baissière) sur fond noir."""
    for x, row in enumerate(df.itertuples()):
        color = _MT5_BULL if row.close >= row.open else _MT5_BEAR
    ...
    step = max(len(df) // 7, 1)
    ax.set_xticks(range(0, len(df), step))
    ax.set_xticklabels([df["time"].iloc[i].strftime("%d %b %H:%M")
                        for i in range(0, len(df), step)],
...
def render_trade_png(ltf: pd.DataFrame, trade: pd.Series, path: Path,
    ...
    """Bougies M15 autour du trade (thème noir MT5), entrée / SL / TP."""
    ...
    fig, ax = plt.subplots(figsize=(12, 6), facecolor=_MT5_BG)
    ax.set_facecolor(_MT5_BG)
    _dark_candles(ax, win)

    ax.axhline(trade["entry"], color="#4d9fff", linewidth=1.3,
               label=f"Entrée {trade['entry']:.5f}")
    ax.axhline(trade["sl"], color="#ff5252", linewidth=1.3, linestyle="--",
               label=f"SL {trade['sl']:.5f}")
    ax.axhline(trade["tp"], color="#4caf50", linewidth=1.3, linestyle="--",
               label=f"TP {trade['tp']:.5f}")

    res = trade["result_r"]
    ax.set_title(f"{trade['pair']} {trade['direction'].upper()} · "
                 f"{trade['zone_kind']} · sortie {trade['exit_kind']} : "
                 f"{res:+.2f} R",
                 fontsize=12, color="#4caf50" if res > 0 else "#ff5252", pad=10)
    leg = ax.legend(loc="upper left", fontsize=8, facecolor="#111111",
                    edgecolor="#333333")
    for t in leg.get_texts():
        t.set_color(_MT5_TEXT)
    fig.tight_layout()
...
def render_equity_png(trades: pd.DataFrame, path: Path) -> None:
    """Courbe d'équité en thème noir."""
    equity = trades["result_r"].cumsum()
    fig, ax = plt.subplots(figsize=(12, 5), facecolor=_MT5_BG)
    ax.set_facecolor(_MT5_BG)
    ax.plot(trades["open_time"], equity, color=_MT5_PRICE, linewidth=1.6)
    ax.axhline(0, color="#666", linewidth=0.8, linestyle="--")
    ax.fill_between(trades["open_time"], equity, 0, where=(equity >= 0),
                    color=_MT5_PRICE, alpha=0.12)
    ax.fill_between(trades["open_time"], equity, 0, where=(equity < 0),
                    color="#ff5252", alpha=0.12)
    ax.set_title(f"Courbe d'équité — {len(trades)} trades — "
                 f"total {equity.iloc[-1]:+.1f} R", fontsize=13,
                 color="#ffffff", pad=10)
    ax.set_ylabel("R cumulés", color=_MT5_TEXT)
    ax.tick_params(colors=_MT5_TEXT, labelsize=8)
    ax.grid(True, color=_MT5_GRID, linewidth=0.6, linestyle=(0, (1, 3)))
    for spine in ax.spines.values():
        spine.set_color("#444444")
